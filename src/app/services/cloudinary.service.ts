...
export class CloudinaryService {
  ...
  getChiotVideo(publicId: string): CloudinaryVideo {
    return this.cld.video(publicId)
  ...
  generateSrcset(publicId: string, widths: number[]): string {
    ...
    const srcsetUrls = widths.map(width =>
      `${baseImage.resize(scale().width(width)).toURL()} ${width}w`
    );
  ...
  getImageUrlsForSrcset(publicId: string, widths: number[], mainMaxWidth: number): { src: string, srcset: string } {
